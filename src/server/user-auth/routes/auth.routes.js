...
function authRoutes(fastify, options, done) {
	...
	fastify.post('/register', async (request, reply) => {
		...
		if (!email || !username || !password) return reply.code(400).send({ error: 'Missing fields' });
		...
		const usernameValidation = authUtils.verifyUsername(username);
		...
		const exists = await userService.userExists(email, username);
	...
	fastify.post('/login', async (request, reply) => {
		...
		if (user.two_fa_enabled) {
			const tempToken = fastify.jwt.sign({ id: user.id, username: user.username, twoFA: true }, JWT_SECRET, { expiresIn: '5m'});
			return reply.send({ message: '2FA required', tempToken});
		}

		const token = fastify.jwt.sign({ id: user.id, username: user.username }, JWT_SECRET, { expiresIn: '1h' });
		reply.send({ message: 'Login successful', token, user: {id : user.id, email: user.email, username: user.username } });
	});
	...
	fastify.post('/login/2fa', async (request, reply) => {
		...
		try {
			...
			const realToken = fastify.jwt.sign({ id: user.id, username: user.username }, JWT_SECRET, { expiresIn: '1h' });
			reply.send({ message: '2FA login successful', token: realToken, user: { id: user.id, email: user.email, username: user.username } });
		} catch {
	...
	fastify.get('/me', { preHandler: verifyToken }, async (request, reply) => {
		reply.send({ id: request.user.id, username: request.user.username });
	});

	fastify.get('/2fa/setup', { preHandler: verifyToken }, async (request, reply) => {
		const secret = require('speakeasy').generateSecret({ name: `Transcendence (${request.user.username})` });
